...
PLAYER_NAME = 'derrell'
When 'A Player does not exist and I enter a his data' do
  user = User.where(name: PLAYER_NAME).first
  unless user.nil?
    id = user.id
    User.delete(id)
  end
  invite_player PLAYER_NAME, 'dd@fake.com'
end

Then 'I should have a new player in the database' do
...
Then 'I should see a message that my Player was created' do
  page.should have_content("User '#{PLAYER_NAME}' created")
end

Then 'I should see the page to invite a new Player' do
  page.should have_button 'Invite Player'
end

Then 'I should send an email to the Player with a link for the Player to log in' do
  mailbox_for('dd@fake.com').size.should == parse_email_count(1)
end

When 'I invite an existing player' do
  invite_player PLAYER_NAME, 'dd@fake.com'
end
